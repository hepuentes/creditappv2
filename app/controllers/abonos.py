...
def crear():
    ...
    try:
        ...
        current_app.logger.error(f"Error en crear abono: {str(e)}")
        flash('Error al cargar la página de abonos. Intente nuevamente.', 'danger')
        return redirect(url_for('abonos.index'))
...
def detalle(id):
    try:
        ...
        current_app.logger.error(f"Error en detalle abono: {str(e)}")
        flash('Error al cargar el detalle del abono.', 'danger')
        return redirect(url_for('abonos.index'))
...
def pdf(id):
    try:
        ...
        current_app.logger.error(f"Error en PDF abono: {str(e)}")
        flash('Error al generar el PDF del abono.', 'danger')
        return redirect(url_for('abonos.index'))
